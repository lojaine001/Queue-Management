...
_ARRIVAL_MODEL_OPTIONS = ["ensemble", "prophet", "lstm", "xgboost"]
_ARRIVAL_MODEL_DEFAULT = ["ensemble"]
...
if "arrival_calibration_enabled" not in st.session_state:
    _qp_ac = str(st.query_params.get("arrival_calibration", "")).strip().lower()
    if _qp_ac in {"1", "true", "yes", "on"}:
        st.session_state["arrival_calibration_enabled"] = True
    elif _qp_ac in {"0", "false", "no", "off"}:
        st.session_state["arrival_calibration_enabled"] = False
    else:
        st.session_state["arrival_calibration_enabled"] = _env_flag("ARRIVAL_CALIBRATION_ENABLED", True)
...
_arrival_calib_toggle = bool(st.session_state.get("arrival_calibration_enabled", True))
...
_selected_models = st.session_state.get("arrival_models") or ["ensemble"]
if not pred_df.empty:
    ...
    _arrival_baked_in = _arrival_calib_active
    if (
        _arrival_calib_toggle
        and not _arrival_calib_active
        and _arrival_calib.get("loaded")
        and not pred_future.empty
    ):
        _k_per_slot = [
            float(_cal_hourly_pre.get(int(pd.Timestamp(t).hour), _cal_global_pre))
    ...
    elif not _arrival_calib_toggle:
        if _arrival_baked_in:
            _arrival_calib_note = (
                "Arrival calibration is toggled <strong>off</strong> in the dashboard, but the selected "
                "<code>ensemble_yhat</code> series was already calibrated when it was saved to the DB. "
                "To fully disable it, rerun <code>ensemble_predict.py</code> with "
                "<code>ARRIVAL_CALIBRATION_ENABLED=0</code> or <code>--arrival-calibration off</code>."
            )
        else:
            _arrival_calib_note = (
                "Arrival calibration is toggled <strong>off</strong> in the dashboard, so the selected raw arrival "
                "model columns are being shown without applying <code>k_global</code> / <code>k_by_hour</code>."
            )
    ...
    _arrival_calib_debug_rows = [
        ...
        {"Field": "Dashboard toggle", "Value": "on" if _arrival_calib_toggle else "off"},
        ...
        {"Field": "Baked into saved series", "Value": "yes" if _arrival_baked_in else "no"},
...
def _sync_arrival_calibration_to_qp():
    st.query_params["arrival_calibration"] = (
        "on" if st.session_state.get("arrival_calibration_enabled", True) else "off"
    )
...
action_refresh, action_predict, action_backtest, action_param, action_days, action_model, action_calib, action_smooth, action_dwell, action_dwell_models = st.columns([1, 1.2, 1.2, 1.7, 1.7, 1.8, 1.2, 1.4, 1.2, 2.0])
with action_refresh:
...
with action_model:
    st.multiselect(
        "Arrivals model(s)",
        options=["ensemble", "prophet", "lstm", "xgboost"],
        format_func=lambda v: {"ensemble": "Ensemble", "prophet": "Prophet", "lstm": "LSTM", "xgboost": "XGBoost"}[v],
        key="arrival_models",
...
with action_calib:
    st.checkbox(
        "Arrival calib",
        key="arrival_calibration_enabled",
        help="Turn multiplicative arrival calibration on/off for the dashboard view. Saved ensemble rows already written to the DB remain baked.",
        on_change=_sync_arrival_calibration_to_qp,
    )
with action_smooth:
    st.select_slider(
...
_cmp_active_models = st.session_state.get("arrival_models", ["ensemble"]) or ["ensemble"]
...
_active_models = st.session_state.get("arrival_models", ["ensemble"]) or ["ensemble"]
