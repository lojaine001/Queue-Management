...
### `.env` (prediction & dashboard)

All runtime knobs live in `Queue-Management/.env` and are loaded by `python-dotenv`.

| Variable | Default | Description |
|---|---|---|
| `DB_HOST` / `DB_PORT` / `DB_NAME` / `DB_USER` / `DB_PASSWORD` | `localhost/5432/iqms/postgres/0000` | PostgreSQL connection |
| `SHOP_OPEN` | `8` | Default opening hour (all days) |
| `SHOP_CLOSE` | `21` | Default closing hour (all days) |
| `SHOP_SCHEDULE_OVERRIDE` | `{}` | JSON dict of per-day overrides — see below |
| `BUCKET_MINUTES` | `3` | Forecast bucket size in minutes |
| `FORECAST_HORIZON_MINUTES` | `60` | Look-ahead window |
| `LOOKBACK_MINUTES` | `60` | LSTM sequence window |
| `ROLLING_WINDOW_MINUTES` | `30` | XGBoost trailing-average window |
| `MIN_REAL_DAYS_FOR_SIM` | `14` | Stop using synthetic history once ≥ N real days exist |
| `MAX_QUEUE_PER_LANE` | `20` | Wait-model cap per lane |
| `MAX_WAIT_MIN` | `60` | Wait-model cap (minutes) |
| `DEFAULT_DWELL_MIN` | `3.0` | Prior service time when no snapshot exists |
| `DWELL_MIN_FLOOR` | `1.0` | Minimum allowed dwell value |
| `DWELL_MAX_CAP` | `10.0` | Maximum allowed dwell value |
| `DEFAULT_LANES` | `2` | Default number of open lanes |
| `SNAPSHOT_MAX_AGE_MIN` | `30` | Snapshots older than this are ignored |
| `W_PROPHET` / `W_LSTM` / `W_XGB` | `0.40/0.30/0.30` | Ensemble weights (must sum to 1) |
| `MODEL_MAX_AGE_HOURS` | `24` | Re-train threshold |

#### Per-day schedule override (`SHOP_SCHEDULE_OVERRIDE`)

A JSON object keyed by weekday integer (`"0"` = Monday … `"6"` = Sunday). Only fields that differ from the defaults need to be specified:

```
SHOP_SCHEDULE_OVERRIDE={"6": {"close_hour": 13, "close_minute": 0}}
```

Supported override keys per day entry: `open_hour`, `open_minute`, `close_hour`, `close_minute`.

The `_day_hours(weekday)` helper in `prediction/core.py` resolves these overrides at call time and returns `(open_hour, open_minute, close_hour, close_minute)`. `is_open()` and `future_open_timestamps()` both call `_day_hours()`, so all schedule-aware code automatically respects the override.
...
### May 2026 — Per-day schedule, dashboard accuracy & robustness

**Fix 20 — Per-day shop hours (`SHOP_SCHEDULE_OVERRIDE`)**

Shop hours were previously global constants. Added `SHOP_SCHEDULE_OVERRIDE` JSON env var (weekday-keyed) and `_day_hours(weekday)` helper in `prediction/core.py`. Both `is_open()` and `future_open_timestamps()` now resolve per-day hours. The dashboard `_today_hours()` wrapper exposes today's effective hours to all dashboard sections (status breakdown, lane history, dwell grid, context pill). A context pill shows the current day's open–close window with an `(override)` badge when the default hours are overridden.
...
**Fix 21 — Live Operations open-hours filter for predictions**

`load_predictions()` returned future rows without filtering by open hours. After loading, the dashboard now applies `is_open()` to `pred_df` so that Live Operations never shows predictions outside the shop's operating window.

**Fix 22 — `compute_wait_estimates` robustness guards**

Two `IndexError` crashes fixed in `prediction/core.py`:
...
| Area | Change |
|---|---|
| `fig_s2` y-axis | Auto-scaled to `max(peak_queue × 1.3, WAIT_ALERT_MIN + 5, 15)` so coloured zones grow with the data |
| `fig_cap` y-axis | Auto-scaled to `max(pred_max × 1.3, effective_cap × 1.3, 8)` |
| Net flow dead-band | Float values clamped to 1 decimal before comparing; "balanced" label used when |arrivals − capacity| ≤ 1.0 (was 0.2) to avoid spurious alerts on rounding noise |
| `_eff_dwell` | Now uses XGBoost dwell prediction for slot 0 (the current 3-min bucket) instead of the mean across all forecast steps |
