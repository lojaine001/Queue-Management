...
def match_boxes(Vehicle_boxes, Pattern_boxes, Vehicles_conf, Patterns_conf, min_iou, Vehicle_classes, Pattern_classes, names={}, TaxiSign=(0.2, 0, 0), debug=True):
    """Match Vehicle with Pattern using Containment Ratio (Aligned with EV App)"""
    vehicles = []
    
    # Logger for traces
    trace_logger = logging.getLogger('Detections')

    for v_idx, (vehicle_box, vehicle_conf, vehicle_class) in enumerate(zip(Vehicle_boxes, Vehicles_conf, Vehicle_classes)):
        patterns = []
        for p_idx, (pattern_box, pattern_conf, pattern_class) in enumerate(zip(Pattern_boxes, Patterns_conf, Pattern_classes)):
            
            # Check Containment Ratio: Area(Intersection) / Area(Pattern)
            xA = max(vehicle_box[0], pattern_box[0])
            yA = max(vehicle_box[1], pattern_box[1])
            xB = min(vehicle_box[2], pattern_box[2])
            yB = min(vehicle_box[3], pattern_box[3])

            interArea = max(0, xB - xA) * max(0, yB - yA)
            patternArea = (pattern_box[2] - pattern_box[0]) * (pattern_box[3] - pattern_box[1])

            containment = interArea / patternArea if patternArea > 0 else 0
            
            p_name = names.get(int(pattern_class), f"ID:{pattern_class}") if names else f"ID:{pattern_class}"

            if containment > min_iou:
                w = pattern_box[2] - pattern_box[0]
                h = pattern_box[3] - pattern_box[1]
                area = w * h 
                ratio = round(w / h, 2) if h != 0 else 0

                # Only handle TaxiSign (class 0 in pattern model)
                if int(pattern_class) == 0:
                    conf_ok = float(pattern_conf) >= TaxiSign[0]
                    area_ok = area >= TaxiSign[1]
                    ratio_ok = ratio >= TaxiSign[2]

                    if conf_ok and area_ok and ratio_ok:
                        patterns.append((pattern_box, pattern_conf, pattern_class, area, ratio, containment))
                        if debug:
                            trace_logger.debug(f"    [OK] ACCEPTED | {p_name:<10} | Conf:{pattern_conf:.2f}>={TaxiSign[0]} | Area:{area:.0f}>={TaxiSign[1]} | Ratio:{ratio:.2f}>={TaxiSign[2]} | Incl:{containment:.2f}>{min_iou}")
                    elif debug:
                        reasons = []
                        if not conf_ok: reasons.append(f"Conf:{pattern_conf:.2f}<{TaxiSign[0]}")
                        if not area_ok: reasons.append(f"Area:{area:.0f}<{TaxiSign[1]}")
                        if not ratio_ok: reasons.append(f"Ratio:{ratio:.2f}<{TaxiSign[2]}")
                        trace_logger.debug(f"    [FAIL] REJECTED | {p_name:<10} | {' | '.join(reasons)} | Incl:{containment:.2f}>{min_iou}")
            elif debug and containment > 0:
                trace_logger.debug(f"Pattern Match | {p_name:<10} | Containment: {containment:.3f} <= {min_iou} (FAIL)")

        vehicles.append((vehicle_box, vehicle_conf, vehicle_class, patterns))
